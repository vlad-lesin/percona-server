...
//#include "Percona-Tokubackup/backup/real_syscalls.h"
#include <regex.h>
#include <rpl_slave.h>
#include <rpl_msr.h>
#include <rpl_rli.h>
...
template <typename T>
class BasicLockablePODWrapper
{
    T &m_lockable;
    void (*m_lock)(T *);
    void (*m_unlock)(T *);
public:
    BasicLockablePODWrapper(T &a_lockable,
                            void (*a_lock)(T *),
                            void (*a_unlock)(T *)) :
        m_lockable(a_lockable),
        m_lock(a_lock),
...
static const char* master_info_file_name = "binlog_pos.txt";
...
static void tokudb_backup_run(THD *thd, const char *dest_dir) {
    int error = 0;
    char master_info_full_file_name[FN_REFLEN];
    ...
    // TODO: return error if file path is truncated
    snprintf(master_info_full_file_name,
             sizeof(master_info_full_file_name),
             "%s%s",
             dest_dir_path,
             master_info_file_name);
    free(dest_dir_path);
